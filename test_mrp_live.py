...
with httpx.Client(follow_redirects=True, timeout=180) as c:
    r = c.post(f"{BASE}/login", data={"username": "admin", "password": "vrm20@9837"})
    print(f"Login: {r.status_code}, cookies={list(c.cookies.keys())}")

    r = c.get(f"{BASE}/api/monitors")
    if r.status_code != 200:
        print(f"List monitors: {r.status_code} - {r.text[:200]}")
        sys.exit(1)
